...
const StudentPayments = () => {
  ...
  const { data: tuitionFees, isLoading } = useQuery({
    queryKey: ['student-tuition-fees'],
    queryFn: studentApi.getTuitionFees,
  });
  ...
  const generateReceipt = (payment: any) => {
    try {
      ...
      const studentName = studentProfile?.user 
        ...
        : 'Non disponible';
      const studentPhone = studentProfile?.user?.phone || 'Non disponible';
      ...
      {isLoading ? (
        <Card>
          <div className="flex items-center justify-center py-12">
            <div className="text-center">
              ...
              <p className="text-gray-600">Chargement des frais de scolarité...</p>
            </div>
          </div>
        </Card>
            ...
            >
              ...
              {fee.payments && fee.payments.length > 0 && (
                <div className="mt-4 pt-4 border-t border-gray-200">
                  <p className="text-sm font-semibold text-gray-700 mb-2">Historique des paiements :</p>
        ...
        <Card>
          <div className="text-center py-12 text-gray-500">
            ...
            <p className="text-lg font-semibold mb-2">Aucun frais de scolarité enregistré</p>
            <p className="text-sm">Contactez l'administration pour plus d'informations</p>
          </div>
        </Card>
      ) : filteredFees && filteredFees.length === 0 && tuitionFees && tuitionFees.length > 0 ? (
        <Card>
          <div className="text-center py-12">
            <FiCheckCircle className="w-16 h-16 mx-auto mb-4 text-green-500" />
            <p className="text-lg font-semibold text-gray-900 mb-2">Tous vos frais sont payés !</p>
            <p className="text-sm text-gray-600 mb-4">Aucun paiement en attente</p>
            <Button
              variant="secondary"
              onClick={() => {
                setSearchQuery('');
        ...
        <Card>
          <div className="text-center py-12 text-gray-500">
            <FiDollarSign className="w-16 h-16 mx-auto mb-4 text-gray-400" />
            <p className="text-lg font-semibold mb-2">Aucun frais trouvé</p>
            <p className="text-sm mb-4">Aucun frais ne correspond à vos critères de recherche</p>
            <Button
              variant="secondary"
              onClick={() => {
                setSearchQuery('');
        ...
        >
          <div className="space-y-4">
            <div className="p-4 bg-gray-50 rounded-lg">
              ...
              <p className="font-semibold text-gray-900">{selectedFee.period}</p>
                ...
                {selectedFee.totalPaid !== undefined && selectedFee.totalPaid > 0 && (
                  <>
                    ...
                    <div className="flex justify-between items-center pt-2 border-t border-gray-200">
                      ...
                      <p className="text-lg font-bold text-purple-600">{formatFCFA(selectedFee.remainingAmount || (selectedFee.amount - selectedFee.totalPaid))}</p>
                    </div>
              ...
              {new Date(selectedFee.dueDate) < new Date() && !selectedFee.isPaid && (
                <div className="mt-3 p-2 bg-red-50 border border-red-200 rounded-lg flex items-center space-x-2">
                  ...
                  <p className="text-sm text-red-800">Attention: Ce paiement est en retard !</p>
                </div>
              )}
            </div>
            ...
            <div className="w-full">
              ...
              <div className="grid grid-cols-2 gap-3 mb-4 w-full">
                {[
                  { value: 'CARD', label: 'Carte', icon: FiCreditCard },
                  { value: 'MOBILE_MONEY', label: 'Mobile Money', icon: FiSmartphone },
                  { value: 'BANK_TRANSFER', label: 'Virement', icon: FiFileText },
                  { value: 'CASH', label: 'Espèces', icon: FiDollarSign },
                ].map((method) => {
                  ...
                  return (
                    <button
                      ...
                      className={`p-4 border-2 rounded-xl transition-all duration-200 flex flex-col items-center justify-center min-h-[100px] w-full ${
                    ...
                    >
                      <Icon className={`w-6 h-6 mb-2 ${
                        paymentMethod === method.value ? 'text-purple-600' : 'text-gray-400'
                      }`} />
                      <p className={`text-xs font-semibold text-center ${
                        paymentMethod === method.value ? 'text-purple-700' : 'text-gray-600'
                      }`}>
                        {method.label}
                      </p>
                    </button>
              ...
              <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg flex items-start space-x-2 mb-4">
